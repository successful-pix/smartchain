...
const title="SmartChain Wallet — Secure Crypto Dashboard";const description="SmartChain is a secure crypto wallet and trading platform.";const site="https://smartchains.online";
...
function Dashboard(){const [splash,setSplash]=useState(true);const p=usePortfolio();const tx=useTransactions(5);const n=useNotifications();const error=tx.error;useEffect(()=>{const id=window.setTimeout(()=>setSplash(false),900);return()=>window.clearTimeout(id)},[]);if(splash)return <SmartChainLoader/>;return <div className="min-h-screen bg-background pb-24"><AppHeader notificationCount={n.unread}/><main className="mx-auto max-w-lg space-y-6 px-4 pt-5"><h1 className="sr-only">SmartChain wallet dashboard</h1><BalanceCard portfolio={p.portfolio} loading={p.markets.isLoading}/><QuickActions/><AssetList positions={p.positions} loading={p.markets.isLoading} error={error?(error as Error).message:null} onRetry={()=>{void p.holdings.refetch();void p.markets.refetch();void tx.refetch()}} onSelect={position=>{window.location.href=`/asset/${position.market.id}`}} onViewAll={()=>{window.location.href="/markets"}}/><RecentActivity transactions={tx.data??[]} loading={tx.isLoading}/></main><InstallPrompt/><BottomNavigation active="home"/></div>}
