...
export function BalanceCard({ portfolio, loading = false }: BalanceCardProps) {
  ...
  return <section aria-label="Available balance" className="bank-card relative isolate overflow-hidden rounded-[12px] border border-border px-5 py-6 sm:px-6">
    ...
    <div className="flex items-start justify-between gap-4"><div>
      ...
      {loading ? <div className="mt-3 h-9 w-44 animate-pulse rounded-md bg-border" /> : <p className="mt-2 font-display text-[34px] font-semibold leading-none tracking-tight sm:text-[40px]">{hidden ? "••••••" : formatFiat(portfolio.availableBalance, portfolio.currency)}</p>}
      <div className="mt-3 flex items-center gap-2"><button type="button" onClick={()=>setHidden(v=>!v)} aria-label={hidden?"Show balance":"Hide balance"} aria-pressed={hidden} className="rounded-md p-1 text-muted-foreground hover:text-primary">{hidden?<EyeOff size={15}/>:<Eye size={15}/>}</button><span className={`rounded-full border px-2 py-0.5 text-[11px] font-medium ${positive?"border-success/25 bg-success/10 text-success":"border-destructive/25 bg-destructive/10 text-destructive"}`}>{formatPercent(portfolio.changePercentToday)} Today</span></div>
    </div><span className="rounded-[6px] border border-primary/30 bg-primary/10 px-2 py-1 text-[10px] font-semibold uppercase tracking-widest text-primary">{portfolio.currency}</span></div>
